...
TABLE = "stores"
CSV_DIR = "../../../local/storesData/stores"
...
def store_location_available(cur, store_id, location, store_locations):
    ...
    return new_id
def import_store(conn, filepath, fname, stores, storeLocations):
    ...
    with conn.cursor() as cur:
        with open(filepath, 'r', encoding='utf-8') as f:
            ...
            for i, row in enumerate(reader, start=2):
                ...
                location_id=store_location_available(cur, store_id, store_str, storeLocations)
...
def main():
    ...
    try:
        ...
        stores, storeLocations = build_lookup_cache(conn)
        ...
        for fname in sorted(csv_files):
            ...
            print(f"\n--- Importing {fname} ---")
            try:
                import_store(conn, filepath, fname, stores, storeLocations)
                conn.commit()
                num += 1
